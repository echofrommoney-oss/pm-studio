...
**Look at what you build:** any work that creates or changes screens must start by reading DESIGN.md, the hallmark SKILL.md and impeccable `reference/craft-floor.md`, state a short design plan (layout, type hierarchy, color use, one memorable element), and finish by screenshotting the result with `python3 scripts/pm_shot.py <url-or-html> .pm-console/shots/<name>.png` and viewing the PNG with Read. Illustration placeholders are styled in the design language, never plain gray boxes.
